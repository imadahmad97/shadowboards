...
def capture_reference_image():
    ...
    output_directory = filedialog.askdirectory(title="Select Directory for Reference Image")
    root.destroy()
...
def crop_reference_object(image):
    r = cv2.selectROI("Select the reference object", image, fromCenter=False, showCrosshair=True)
    if r:
...
if __name__ == "__main__":
    known_size_cm = float(input("Enter the real-world size of the reference object (in cm): "))
